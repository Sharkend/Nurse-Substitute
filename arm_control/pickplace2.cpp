...
int main(int argc, char** argv)
{
  ...
  // are used interchangably.
    ...
    ROS_INFO_NAMED("tutorial", "Visualizing plan 1 (pose goal home) %s", success ? "" : "FAILED");

    move_group_interface_arm.move();
    
    // 1.5 Manual: Move to pick position
    move_group_interface_arm.setJointValueTarget(move_group_interface_arm.getNamedTargetValues("pick"));
    
    ROS_INFO_NAMED("tutorial", "Visualizing plan 1 (pose goal pick) %s", success ? "" : "FAILED");
    ...
    target_pose1.position.x = 0.037441;
    target_pose1.position.y = 0.721189;
    target_pose1.position.z = 1.230452;
    ...
    target_pose1.position.x = -0.3;//target_pose1.position.x - 0.6;
